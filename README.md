...
## Session Manager
This is a python script that will be ran on the seperate linux computer. Preferably, Ubuntu, you will run sessionmanager.py on your linux installation, and for now, you will need to type in your correct credentials. This script is used to monitor the 'session_data' table inside the SQL database, and it will check each active sessions's expiration time to make sure that it is still valid. If a session is expired, it will delete the row. For now, it is ran under a while loop, and is made to run for an extensive amount of time, but later on it might be a scheduled task to be ran.
